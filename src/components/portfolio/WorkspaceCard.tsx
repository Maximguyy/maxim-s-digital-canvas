...
const mcpServers = ["Supabase", "Playwright", "Mobile MCP", "ADB", "Maestro", "Figma", "Sequential Thinking Continuous"];
const techStack = [{
  category: "Core",
  items: [{
    name: "TypeScript 5.7.0",
    icon: SiTypescript
  }, {
    name: "Node.js ≥18.0.0",
    icon: SiNodedotjs
  ...
  items: [{
    name: "Playwright 1.57.0",
    icon: TestTube
  }, {
    name: "@playwright/test 1.57.0",
    icon: TestTube
  }]
}, {
  category: "Execution",
  ...
  return <motion.div initial={{
    ...
    duration: 0.4
      ...
      <div className="border border-border rounded-2xl bg-card/50 backdrop-blur-sm hover:border-primary/50 transition-all duration-300 overflow-hidden">
        {/* Header - Clickable */}
        <button onClick={() => setIsExpanded(!isExpanded)} className="w-full text-left p-6 group">
          <div className="flex items-start justify-between gap-4 mb-4">
            ...
            <div className="flex items-center gap-2">
              <a href="https://github.com/Maximguyy/RN-Workspace" target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="p-2 rounded-full bg-secondary/50 text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {["MCP Servers", "Claude Code", "ADB", "Maestro", "Automatisation"].map(tag => <Badge key={tag} variant="secondary" className="text-xs font-normal rounded-full">
                {tag}
              </Badge>)}
          </div>
              ...
              <div className="px-6 pb-6 pt-2 border-t border-border/30 space-y-6">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {mcpServers.map(server => <Badge key={server} variant="outline" className="text-xs font-normal">
                        {server}
                      </Badge>)}
                  </div>
                </div>
                ...
                setStackExpanded(!stackExpanded);
                  ...
                  duration: 0.2
                        ...
                        <div className="px-4 pb-4 grid grid-cols-1 sm:grid-cols-3 gap-4">
                          {techStack.map(category => <div key={category.category} className="space-y-2">
                          ...
                          return <div key={item.name} className="flex items-center gap-2 text-xs text-muted-foreground">
