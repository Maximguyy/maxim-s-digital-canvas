...
function CountUpKPI({ end, suffix = "", prefix = "" }: { end: number; suffix?: string; prefix?: string }) {
  const { formattedCount, ref } = useCountUp({ end, suffix, prefix, duration: 2000 });
  return <div ref={ref}>{formattedCount}</div>;
}

const kpis = [
  { label: "RDV générés", value: 280, suffix: "+" },
  { label: "CA généré", value: 40000, suffix: "€" },
  { label: "ROI moyen", value: 7.2, suffix: "x", isDecimal: true },
  { label: "CPA moyen", value: 25, suffix: "€" },
];

const strategies = [
  "Ciblage broad local (Le Havre, Rouen, Lens) — rayon 20-30km",
  "Lead forms natifs + Landing page custom optimisée",
  "5+ itérations A/B testées (headlines, CTA, formulaire, social proof)",
];

const creativeStats = [
  { value: "160", label: "créatives testées sur 4 mois" },
  { value: "40", label: "vidéos (avis clients + motivationnels)" },
  { value: "80", label: "visuels statiques (carrousels + images)" },
  { value: "40", label: "variations copies/angles" },
];

const optimizationSteps = [
  "Lancement 2-3 créatives/jour",
  "24h d'apprentissage algo",
  "Analyse performance",
  "Kill si CPA > 30€ | Scale si CPA < 20€",
];

const winnerFormats = [
  { icon: Video, label: "Vidéos avis clients", note: "best performer" },
  { icon: Film, label: "Contenus motivationnels", note: "famille, santé, économies" },
  { icon: BarChart3, label: "Carrousels social proof", note: "" },
];
...
const techStack = [
  {
    ...
    items: [
      { icon: SiFastapi, name: "FastAPI (webhooks)" },
      { icon: SiPython, name: "Python (automations)" },
    ],
  },
  {
    category: "Bases de données",
    items: [
      { icon: SiRedis, name: "Redis (cache conversations)" },
      { icon: SiPostgresql, name: "PostgreSQL (leads persistants)" },
    ],
  },
  {
    category: "IA & Messaging",
    items: [
      { icon: Bot, name: "Claude API (qualification)" },
...
const learningsSuccess = [
  "Vidéos authentiques > Contenu scripté",
  "Broad targeting local > Micro-ciblage",
  "Volume créatif > Perfection d'une seule ad",
  "Formulaires natifs > Landing pages (moins de friction)",
];

const learningsConstraints = [
    ...
    <div className="space-y-12">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {kpis.map((kpi, index) => (
            <GlassCard
              key={kpi.label}
              className="p-4 md:p-6 opacity-0 animate-fade-in text-center"
            ...
            >
              <div className="text-2xl md:text-3xl font-bold text-primary mb-2">
                {kpi.isDecimal ? (
                  ...
                  <CountUpKPI end={kpi.value} suffix={kpi.suffix} />
                )}
              </div>
      ...
      <div>
        ...
        <p className="text-muted-foreground mb-8 text-center">
          Gestion de 3 comptes Meta Ads locaux | Budget : 1 800€/mois (600€/centre)
        </p>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          {/* Production créative */}
          <GlassCard className="p-6 opacity-0 animate-fade-in" style={{ animationDelay: "0.3s" } as React.CSSProperties}>
            <h4 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
        ...
        <GlassCard className="p-6 mt-6 opacity-0 animate-fade-in" style={{ animationDelay: "0.4s" } as React.CSSProperties}>
            ...
            {optimizationSteps.map((step, idx) => (
              <div key={idx} className="flex flex-col md:flex-row items-center gap-2">
                ...
                {idx < optimizationSteps.length - 1 && (
                  <ArrowDown className="h-4 w-4 text-primary md:rotate-[-90deg]" />
                )}
              </div>
            ))}
          </div>
      ...
      <div>
        ...
        <p className="text-muted-foreground mb-8 text-center">
          Qualification & nurturing 100% automatisés via WhatsApp
        </p>

        {/* Stack Technique */}
        <GlassCard className="p-6 mb-6 opacity-0 animate-fade-in" style={{ animationDelay: "0.3s" } as React.CSSProperties}>
          <h4 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {techStack.map((category) => (
              <div key={category.category} className="p-3 rounded-lg bg-secondary/30">
                ...
                <div className="space-y-2">
                  {category.items.map((item) => {
                    ...
                    return (
                      <div key={item.name} className="flex items-center gap-2 text-foreground text-xs">
        ...
        <GlassCard 
          className="p-6 opacity-0 animate-fade-in cursor-pointer" 
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <motion.div
              animate={{ rotate: flowsExpanded ? 180 : 0 }}
      ...
      <div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {/* Ce qui a marché */}
          <GlassCard className="p-6 opacity-0 animate-fade-in" style={{ animationDelay: "0.4s" } as React.CSSProperties}>
            <h4 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
              <CheckCircle2 className="h-5 w-5 text-primary" />
              Ce qui a marché
            </h4>
            <ul className="space-y-3">
              {learningsSuccess.map((item, idx) => (
          ...
          {/* Contraintes gérées */}
          <GlassCard className="p-6 opacity-0 animate-fade-in" style={{ animationDelay: "0.5s" } as React.CSSProperties}>
            <h4 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
              <AlertTriangle className="h-5 w-5 text-amber-500" />
              Contraintes gérées
            </h4>
            <ul className="space-y-3">
              {learningsConstraints.map((item, idx) => (
                <li key={idx} className="flex items-start gap-3 text-sm">
